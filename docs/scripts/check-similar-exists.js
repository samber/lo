...
helpers.forEach((h) => {
  ...
  (h.variantHelpers || []).forEach((ref) => {
    ...
    if (!byFullKey.has(key)) {
      hasError = true;
      console.error(`Missing variant helper reference from ${h.fileName} -> ${key}`);
    }
  });
});

if (hasError) process.exit(1);
console.log('OK: all similarHelpers/variantHelpers references point to existing helpers.');
