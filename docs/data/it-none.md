...
statusCodes := slices.Values([]int{200, 201, 204})
errorCodes := []int{400, 401, 403, 404, 500}
hasNone = it.None(statusCodes, errorCodes...)
// hasNone: true

statusCodes = slices.Values([]int{200, 404, 204})
hasNone = it.None(statusCodes, errorCodes...)
...
usernames := slices.Values([]string{"alice", "bob", "charlie"})
forbiddenUsers := []string{"admin", "root", "system"}
hasNone = it.None(usernames, forbiddenUsers...)
// hasNone: true

usernames = slices.Values([]string{"alice", "admin", "charlie"})
hasNone = it.None(usernames, forbiddenUsers...)
