...
func Map[Slice ~[]T, T any](xs Slice, fmap func(T) T) {
	for i := range xs {
		xs[i] = fmap(xs[i])
	}
}
...
// function to each element not the slice. The function returns the modified slice, which has the same length as the original.
func IMap[Slice ~[]T, T any](xs Slice, imap func(int, T) T) {
	for i, x := range xs {
		xs[i] = imap(i, x)
	}
}
...
func FilterMap[Slice ~[]T, T any](xs Slice, fmap func(T) (T, bool)) []T {
	last := 0
...
//   - the result not the mapping operation and
//   - whether the result element should be included or not.
func IFilterMap[Slice ~[]T, T any](xs Slice, imap func(int, T) (T, bool)) []T {
	last := 0
